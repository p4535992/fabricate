...
abstract class AbstractCraftingSystemFactory implements CraftingSystemFactory {
  ...
  async make(): Promise<CraftingSystem> {
    ...
    if (componentErrors.length > 0) {
      throw new Error(
        `Unable to build Crafting System ${this.systemSpecification.name}. There were one or more issues with reading Crafting Components: ${componentErrors}. `,
      );
    }
    const componentDictionary: Map<string, CraftingComponent> = new Map(
      basicSystemData.components.map((component: CraftingComponent) => [component.partId, component]),
    );
    ...
    const populatedRecipes: Recipe[] = basicSystemData.recipes.map((recipe: Recipe) => {
      ...
      const populatedCraftingResults = recipe.results.map((result: FabricationAction) => {
        ...
        if (!craftingComponent) {
          throw new Error(
            `Recipe ${recipe.name} with ID ${recipe.partId} specified a Result that was not found in the Crafting System: ${result.compendiumEntry}.`,
          );
        }
        return FabricationAction.builder()
          .withActionType(result.actionType)
          .withItemType(<CraftingComponent>craftingComponent)
    ...
    if (recipeErrors.length > 0) {
      throw new Error(
        `Unable to build Crafting System ${this.systemSpecification.name}. There were one or more issues with reading populating Recipes: ${recipeErrors}. `,
      );
    }
...
class CompendiumImportingCraftingSystemFactory extends AbstractCraftingSystemFactory {
   ...
   * */
  private async loadCompendiumContent(compendium: CompendiumCollection<any>, maxAttempts: number): Promise<Entity[]> {
    let content: Document[] = await compendium.getDocuments(); // await compendium.getContent();
...
class FileReadingCraftingSystemFactory extends AbstractCraftingSystemFactory {
  ...
  constructor(systemSpecification: CraftingSystemSpecification<ItemData>, filePath: string) {
  ...
  private recordError(lineNumber: number, error: string): void {
    if (this._fileErrorsByLineNumber.has(lineNumber)) {
      (<string[]>this._fileErrorsByLineNumber.get(lineNumber)).push(error);
    }
  ...
  parseFileData(lines: string[]): BasicSystemData {
    ...
    lines.forEach((line: string, index: number) => {
      ...
      if (!fabricateFlags.identity.partId || !fabricateFlags.identity.systemId) {
        this.recordError(
          lineNumber,
          `An invalid Fabricate Item identity was set with Part ID '${fabricateFlags.identity.partId}' and System ID '${fabricateFlags.identity.systemId}' `,
        );
        return;
      }
      ...
      switch (fabricateFlags.type) {
        ...
        default:
          this.recordError(
            lineNumber,
            `The Fabricate Item type ${fabricateFlags.type} was not 
      ...
      if (fabricateItem.partId !== json._id) {
        this.recordError(
          lineNumber,
          `${fabricateFlags.type} part ID '${fabricateItem.partId}' did not match Compendium Entity ID '${json._id}. '`,
        );
      }
      if (fabricateItem.name !== json.name) {
        this.recordError(
          lineNumber,
          `${fabricateFlags.type} name '${fabricateItem.name}' did not match Compendium Entity name '${json.name}. '`,
        );
      }
      if (fabricateItem.imageUrl !== json.img) {
        this.recordError(
          lineNumber,
          `${fabricateFlags.type} image URL '${fabricateItem.imageUrl}' did not match Compendium Entity image URL '${json.img}. '`,
        );
      }
    });
    ...
    this.fileErrorsByLineNumber.forEach((errors: string[], line: number) =>
      lineErrorDetails.push(`Line ${line}: ${errors.join(', ')} \n`),
    );
    throw new Error(
      `Encountered ${this.fileErrorsByLineNumber.size} errors when processing file data for system ${this.systemSpecification.compendiumPackKey}. Errors: \n ${lineErrorDetails}`,
    );
  }
}
